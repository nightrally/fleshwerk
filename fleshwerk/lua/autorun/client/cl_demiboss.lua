...
function ZCorruptionPact()
    ...
    local function Zombie(name, desc, img, cost)
        ...
        costlbl:SetTextColor(NZSCOLOR_CREDITS)
        ...
        btn.DoClick = function()
            if lp:GetNWInt("ZCorruptCreds") < cost or GAMEMODE:GetWave() == 0 or not GAMEMODE:GetWaveActive() or lp:GetZombieClassTable().Demiboss or lp:GetZombieClassTable().Boss then
                ...
                elseif lp:GetZombieClassTable().Demiboss or lp:GetZombieClassTable().Boss then
    ...
    Zombie("Flesh Forager", "The Flesh Forager is slow and weak but has the ability to throw Bonemesh bombs.", "nightrally/killicons/zombies/fleshforager", 1500)
end
